...
void DrawProjection(int pad,TCanvas& canvas,const std::vector<Hit>& hits,
                    const std::vector<Hit>& selectedHits,
                    bool showSelected,
...
void DrawEvent(TCanvas& canvas,TTree& hitsTree,TTree& fitTree,TTree* segmentTree,TTree* selectedTree,int event,const std::string& colourMode) {
    ...
    const bool showSelected=!forceAll&&drawMode!="all"&&drawMode!="off"&&drawMode!="selected"&&!selectedHits.empty()&&selectedHits.size()<hits.size();
    if(drawMode=="selected") hits=selectedHits;
    auto segments=segmentTree?ReadSegments(*segmentTree,event):std::vector<Segment>();canvas.Clear();canvas.Divide(2,2);
    DrawProjection(1,canvas,hits,selectedHits,showSelected,segments,fit,0,1,2,"XY: charge, black=selected, red=fit, green=MC","x","y",drawMode);
    DrawProjection(2,canvas,hits,selectedHits,showSelected,segments,fit,0,2,0,"XZ: charge, black=selected, red=fit, green=MC","x","z",drawMode);
    DrawProjection(3,canvas,hits,selectedHits,showSelected,segments,fit,1,2,1,"YZ: charge, black=selected, red=fit, green=MC","y","z",drawMode);
...
Options:
  ...
  MODE=selected        Only fibres used by the fit
    ...
    std::string eventOption="first",colourMode="log";
    for(int i=2;i<argc;++i){std::string argument=argv[i];auto equal=argument.find('=');if(equal==std::string::npos){std::cerr<<"Options must use KEY=value: "<<argument<<"\n";return 2;}
        std::string key=argument.substr(0,equal),value=argument.substr(equal+1);if(key=="EVENT")eventOption=value;else if(key=="MODE")colourMode=value;else{std::cerr<<"Unknown option: "<<key<<"\n";return 2;}}
    const bool requestedEvent=eventOption!="first";
    const int initialEvent=requestedEvent?std::stoi(eventOption):0;
    if(colourMode!="log"&&colourMode!="linear"&&colourMode!="off"&&colourMode!="all"&&colourMode!="all-log"&&colourMode!="all-linear"&&colourMode!="selected") {std::cerr<<"Invalid display MODE\n";return 2;}
    int rootArgc=1;
    ...
    TCanvas canvas("global_fit_display","Global light fit display",1400,900);DrawEvent(canvas,*hits,*fits,segments,selected,current,colourMode);
    ...
    while(std::cout<<"> "&&std::cin>>command&&command!="q"){
        ...
        DrawEvent(canvas,*hits,*fits,segments,selected,current,colourMode);
    }return 0;
}
